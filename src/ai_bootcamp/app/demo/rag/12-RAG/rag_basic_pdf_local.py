...
def create_embeddings():
    """단계 3: 임베딩 생성 (로컬 모델)"""
    print("\n=== 단계 3: 임베딩 생성 ===")
    print("로컬 HuggingFace 임베딩 모델을 사용합니다...")
...
def create_chain(retriever, prompt, llm):
    ...
    if llm is None:
        print("LLM이 초기화되지 않아 체인을 생성할 수 없습니다.")
        return None
...
def main():
    ...
    print("RAG 기본 구조 이해하기 - 텍스트 문서 기반 RAG 시스템 (로컬 모델 버전)")
    print("=" * 60)
    ...
    try:
        ...
        if chain is None:
            print("\n" + "=" * 60)
            print(
                "임베딩 모델은 로컬에서 실행되었지만, LLM을 사용하려면 OpenAI API 키가 필요합니다."
            )
